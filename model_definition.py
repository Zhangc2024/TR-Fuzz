...
from data_process import input_token_index, target_token_index
# ------------------------------------------------------------------
# Transformer Parameters
d_model = 512  # Embedding Size     
d_ff = 2048    # FeedForward dimension                   
d_k = d_v = 64  # dimension of K(=Q), V                  
n_layers = 2  # number of Encoder o f Decoder Layer        
n_heads = 8  # number of heads in Multi-Head Attention    
src_vocab_size = len(input_token_index)
tgt_vocab_size = len(target_token_index)
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=60):
        ...
        # 一个全0的张量，  [max_len, d_model]
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        pe = pe.unsqueeze(0).transpose(0, 1)
        # [1, max_len, d_model].transpose(0, 1) --> [max_len, 1, d_model]
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):        
        '''
        Q: [batch_size, n_heads, len_q, d_k]        # n_heads * d_k = d_model
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        ...
        enc_outputs: [batch_size, src_len, d_model]
        dec_self_attn_mask: [batch_size, tgt_len, tgt_len]     dec_inputs做self-attention的mask矩阵
